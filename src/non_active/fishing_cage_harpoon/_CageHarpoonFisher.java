...
import disabled.fishing_cage_harpoon.corsair.WalkBankToFishing;
import disabled.fishing_cage_harpoon.corsair.WalkFishingToBank;
import disabled.fishing_cage_harpoon.karamja.*;
...
public class _CageHarpoonFisher extends PollingScript<ClientContext> implements MessageListener, PaintListener {
    ...
    public void start() {
        ...
        if (!catchName.equalsIgnoreCase("lobster")) {
            ...
            String dragonHarpoonResponse = CommonActions.promptForSelection("Harpoon Type", "Using Dragon Harpoon?", new String[]{"Yes", "No"});
            if (dragonHarpoonResponse.equals("Yes")) {
                taskList.add(new HarpoonSpecial(ctx));
            }

            if (CommonActions.promptForYesNo("Drop Tuna", "")) {
                taskList.add(new DropTuna(ctx));
            }
...
*/
            ...
            taskList.add(new disabled.fishing_cage_harpoon.fishing_guild.GuildWalkFishingToBank(ctx));
        }
    ...
    @Override
    public void poll() {
        if (getRuntime() - lastCameraMove > (AntibanTools.getRandomInRange(50, 120) * 1000)) {
            int pitchOffset = ctx.camera.pitch() <= 75 ? (75 - ctx.camera.pitch() + AntibanTools.getRandomInRange(0, 10)) : ctx.camera.pitch() == 99 ? AntibanTools.getRandomInRange(-10, 0) : AntibanTools.getRandomInRange(-10, 10);
